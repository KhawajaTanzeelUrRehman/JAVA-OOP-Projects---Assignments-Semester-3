...
public class CredentialsforAdmin extends JFrame implements ActionListener{
    JLabel headingLabel,usernameLabel,password;
    JButton Back,check, update;
    Choice choice;
    JTextField PasswordtextField;
    public CredentialsforAdmin(){
        headingLabel = new JLabel("Credentials For Admin");
        ...
        try {
            ...
            ResultSet result = databaseConnection.statement.executeQuery("Select * from adminlogin");
    ...
    public void actionPerformed(ActionEvent ae){
        if (ae.getSource() == check) {// The check option will check all the customer information
            ...
            try {
                ...
                //creating and initializing a local variable of class ResultSet.
                ResultSet result_1 = databaseConnection.statement.executeQuery("Select * from adminlogin where userid='" + s1 + "'");
                //using a while loop to jump from column to column and using next() to jump from row to row.
                while (result_1.next()) {
            ...
            try {
                ...
                databaseConnection.statement.executeUpdate("update adminlogin set password = '" + s2 + "'where userid = '" + s1 + "'");
